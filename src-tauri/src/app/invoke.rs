...
#[derive(serde::Deserialize)]
pub struct GetStatsOpts {
    #[serde(rename = "projectPaths", default)]
...
#[command]
pub async fn get_stats(app: AppHandle, opts: GetStatsOpts) -> Result<serde_json::Value, String> {
    let stats = rtk::get_all(opts.project_paths);
    rtk::refresh_tray(&app);
    Ok(stats)
}

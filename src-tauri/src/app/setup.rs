...
pub fn set_system_tray(
    ...
    let savings = MenuItemBuilder::with_id("savings", "Loading…")
        .enabled(false)
        .build(app)?;
    ...
    let menu = if allow_multi_window {
        MenuBuilder::new(app)
            .items(&[&savings, &new_window, &hide_app, &show_app, &quit])
            .build()?
    } else {
        MenuBuilder::new(app)
            .items(&[&savings, &hide_app, &show_app, &quit])
            .build()?
    };

    // Keep the savings item reachable so rtk::update_tray can refresh its text.
    app.manage(crate::rtk::SavingsTrayItem(savings.clone()));
    ...
    // Show the colored app icon in the menu bar / tray. (Rendering it as a macOS
    // "template" flattens it to a solid silhouette — the blank square — so don't.)
    let _ = tray_icon_path;
